...
| C-013 | Phoenix Action Gate usa `nvidia/Nemotron-3_5-Lightning` para generar propuestas gobernables | RUNTIME_OBSERVED | E-012, E-015, E-022 y E-024; ejecución real en Token Factory, sin dispatch |
| C-014 | Phoenix Action Gate llama a Nebius Token Factory en runtime | RUNTIME_OBSERVED | E-011, E-012, E-015, E-022 y E-024 |
| C-015 | Phoenix Action Gate supera de forma general a gates competentes | INSUFFICIENT_EVIDENCE | Trials 002/003 y un caso LIVE son favorables bajo límites declarados; no autorizan superioridad general |
...
| C-019 | La regresión local ampliada del producto pasa 52/52 tests | TEST_EXECUTED | E-025; incluye single-turn, gate, reversibilidad, plan compacto, stale-state, baseline state-aware, lineage, fail-closed y panel |
...
| C-028 | El adaptador `compact-plan/0.2` puede expandir una propuesta semántica compacta de Nemotron al contrato canónico de Plan Gate preservando dependencias, estado explícito y evidence lineage | RUNTIME_OBSERVED | E-024 + E-025; observado en un plan LIVE y cubierto por regresión, sin claim de generalidad |

## Formulación pública permitida hoy

> Phoenix Action Gate is an experimental governed action pipeline for AI agents. Live NVIDIA Nemotron proposals can be evaluated without dispatch. In a bounded live multi-action run, Nemotron generated a four-step plan that a declared state-aware baseline left at REVIEW, while Phoenix changed the plan to DENY after detecting that downstream evidence depended on a source version changed by a later step. Preregistered fixtures separately reproduce stale-state and evidence-lineage discriminants under declared baseline boundaries.
